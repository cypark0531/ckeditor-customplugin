...
import MapApiView from './mapapiview';
import GoogleMapObj from './googleMapObj';
import { toWidget } from '@ckeditor/ckeditor5-widget/src/utils';
import BalloonPanelView from '@ckeditor/ckeditor5-ui/src/panel/balloon/balloonpanelview';
import '../theme/style.css';
export default class MapApiUi extends Plugin {
    ...
    init() {
        ...
        editor.ui.componentFactory.add( 'googleMap', locale => {
            ...
            const mapView = new MapApiView( locale );

            balloon.content.add( mapView );
			const executeAction = () => {
				if (!balloon.isVisible) {
					if (!this.googleMapObj) {
						this.googleMapObj = new GoogleMapObj(editor);
						this.googleMapObj.init();
					}
					balloon.pin({
						target: document.getElementById(this.editor.config.get("mapapi").targetButtonId) || buttonView.element,
						positions: [ this.arrowPin ]
					} );
				} else {
					balloon.hide();
				}
            ...
            buttonView.on('execute', executeAction);
			console.log(self);
			self.executeAction = executeAction;
			this.listenTo( mapView, 'save', ( evt, candidate, item )  => {
				const imageUtils = editor.plugins.get( 'ImageUtils' );
				let url = this.googleMapObj.getImgUrl();
				this.googleMapObj.getImgBlob(url).then(file=>{
					this.editor.execute('uploadImage', { file });
					balloon.hide();
				})
			});
			this.listenTo( mapView, 'cancel', () => {
				balloon.hide();
